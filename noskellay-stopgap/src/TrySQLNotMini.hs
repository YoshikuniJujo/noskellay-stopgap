...
allEvents = withSQLite ("foo_ng" ++ ".sqlite3") $ \db -> do
	selectAll db

allEvents'' :: String -> [Value] -> Int -> IO ([Signed.E], String)
allEvents'' wh vs lmt = withSQLite ("foo_ng" ++ ".sqlite3") $ \db -> do
...
selectEventWithId idnt = withSQLite ("foo_ng" ++ ".sqlite3") $ \db -> do
